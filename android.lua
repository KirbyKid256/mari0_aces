...
function androidLoad()
	...
	if love.filesystem.getInfo("skin.zip") then
		local success = love.filesystem.mount("skin.zip", "alesans_entities", true)
	end

	if love.filesystem.getInfo("skin.json") then
		skinjsonfile = "skin.json"
	end
	if love.filesystem.getInfo("skin.png") then
		skinimgfile = "skin.png"
	end
