...
function changescale(s, fullscreen)
	if android or (s == 5) then
		...
		uispace = math.floor(width*16*scale/4)
		love.window.setMode(width*16*scale, height*16*scale, {fullscreen=fullscreen, vsync=vsync, msaa=fsaa, resizable=window_resizable, minwidth=width*16, minheight=height*16, highdpi=false, usedpiscale=false}) --27x14 blocks (15 blocks actual height)
